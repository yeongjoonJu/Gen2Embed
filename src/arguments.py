...
class ModelArguments:
    ...
    prompt_tuning: bool = field(
        default=False, metadata={"help": "prompt tuning"}
    )
    num_crops: int = field(
        default=16,
...
class DataArguments:
    ...
    use_text_data: bool = field(
        default=False, metadata={'help': 'use text data'}
    )
    Q_prompt: str = field(
        default=None, metadata={"help": "Summarize the provided input with the core intent of a given instruction in one word."}
    )
    D_prompt: str = field(
        default=None, metadata={"help": "Given an image, summarize the provided image in one word. Given only text, describe the text in one word."}
    )
...
class TrainingArguments(TrainingArguments):
    ...
    from_ckpt: str = field(
        default=None, metadata={"help": "load model from checkpoint"}
    )
    ...
    joint_training: bool = field(
        default=False, metadata={"help": "joint training with next token prediction"}
    )
    use_batch_sampling: bool = field(
        default=False, metadata={'help': 'use batch sampling for training'}
    )
    hard_negatives: int = field(
        default=0, metadata={"help": "The number of hard negative samples"}
    )
