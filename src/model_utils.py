...
def Llava_NEXT_process_fn(model_inputs: dict, processor, max_length=None, system_prompt=None, role=None):
    input_ids, pixel_values, image_sizes, image_grid_thw = [], [], [], []
    texts, images = model_inputs['text'], model_inputs['image']
    image_exists = False
    
    if 'system' in model_inputs and model_inputs['system'] is not None:
        system_prompt = model_inputs['system']
        
    # 1. iterate each pair and process (since processors do not support batch processing)
    for text, image in zip(texts, images):
        if role is not None and role=='Q':
            if image is None:
                text += '\nRepresent the given text in one word.'
            else:
...
def Phi3V_process_fn(model_inputs: dict, processor, max_length=None, system_prompt=None, role=None):
    input_ids, pixel_values, image_sizes, image_grid_thw = [], [], [], []
    texts, images = model_inputs['text'], model_inputs['image']
    ...
    if image_exists:
        ...
        inputs['pixel_values'] = pixel_values # torch.zeros(input_ids.shape[0], 1,1,1)
        inputs['image_sizes'] = image_sizes #torch.ones(input_ids.shape[0], 2)

    return inputs


def Qwen2_VL_process_fn(model_inputs: dict, processor, max_length=None, system_prompt=None, role=None):
    # if 'answer' in model_inputs:
    #     return Qwen2_VL_process_fn_jointly(model_inputs, processor, max_length=max_length)
    ...
    if 'counts' in model_inputs:
        inputs.update({'counts': model_inputs['counts']})

    return inputs


def Qwen2_VL_process_fn_jointly(model_inputs: dict, processor, max_length=512):
    texts, images = model_inputs['text'], model_inputs['image']
    answers = model_inputs['answer']
    ...
    for text, image, answer in zip(texts, images, answers):
        if image is None:
        ...
        if answer:
            ...
            ans_ids = processor.tokenizer(answer, add_special_tokens=False)['input_ids']
        ...
        if image is None:
            inputs = processor(
                text=[full], images=None, return_tensors="np",
            ...
            inputs = processor(
                images=[image], text=[full], return_tensors="np",
                max_length=max_length, truncation=True
            )
            pixel_values.append(inputs['pixel_values'])
            image_grid_thw.append(inputs['image_grid_thw'])
    ...
    answer_lens = torch.tensor(answer_lens, device=device)
    start_idx = real_len - answer_lens
    
    pos = torch.arange(L, device=device).unsqueeze(0).expand(B, -1)
    mask = (pos >= start_idx.unsqueeze(1)) & (pos < real_len.unsqueeze(1))
    ...
    if image_exists:
        ...
        enc['pixel_values'] = torch.zeros(input_ids.shape[0], 1)
        enc['image_grid_thw'] = [None] * input_ids.shape[0]
