...
from torch.utils.data import DataLoader, DistributedSampler, Dataset, BatchSampler
...
from src.dataset import SameSubsetBatchSampler
...
class MMEBTrainer(Trainer):
    ...
    def get_train_dataloader(self):
        if not self.args.use_batch_sampling:
        ...
        # Build your same‐subset batch sampler on the *effective* batch size
        # that Trainer would have used automatically.
        global_bs = self.args.train_batch_size * self.args.gradient_accumulation_steps # * getattr(self.args, "world_size", 1)
        
        sampler = SameSubsetBatchSampler(
...
#     # from accelerate.data_loader_utils import BatchSamplerShard
#     sampler = BatchSamplerShard(
#         sampler,
#         num_processes=torch.distributed.get_world_size(),
#         process_index=torch.distributed.get_rank(),
#         split_batches=True,   # makes each GPU see exactly global_bs/world_size = per_step_bs
#     )
# else:
...
class GradCacheLateProcessTrainer(MMEBTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        loss_fn = loss_fn_cls(temperature=self.model.temperature)#, n_target=self.args.hard_negatives)
        process_fn = functools.partial(process_vlm_inputs_fns[self.args.model_backbone], \
            processor=self.processing_class, max_length=self.max_length)
