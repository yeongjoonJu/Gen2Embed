...
class GradCache:
    ...
    def forward_no_grad(
        ...
        if cand_counts:
            # assert model_reps.size(0)==sum(cand_counts), f"{model_reps.size(0)}!={sum(cand_counts)}"
            return model_reps, rnd_states, cand_counts
        else:
    ...
    def cache_step(
        ...
        **loss_kwargs
        ...
        # qry_len = len(model_inputs[0]['qry']['text'])
        # tgt_len = len(model_inputs[1]['tgt']['text'])
        
        qry_system_prompt = None
        tgt_system_prompt = None        
        if 'system' in model_inputs[0]['qry']:
            qry_system_prompt = model_inputs[0]['qry']['system']
            del model_inputs[0]['qry']['system']
        if 'system' in model_inputs[1]['tgt']:
            tgt_system_prompt = model_inputs[1]['tgt']['system']
            del model_inputs[1]['tgt']['system']
        ...
        if self.process_fn:
            ...
            _model_inputs = []
            for g, arg_group in enumerate(model_inputs):
                _arg_groups = []
                for key2val_dict in arg_group:
                    _key2val_dict = {}
                    for arg_key, arg_val in key2val_dict.items():
                        if g==0:
                            _key2val_dict[arg_key] = self.process_fn(arg_val, system_prompt=qry_system_prompt, role='Q')
                        else:
                            _key2val_dict[arg_key] = self.process_fn(arg_val, system_prompt=tgt_system_prompt, role='D')
        ...
        for model, x in zip(self.models, model_inputs):
            model_out = self.forward_no_grad(model, x)
            
            model_reps = model_out[0]
            rnd_states = model_out[1]
    ...
    #     use_hard_neg = (qry_len != tgt_len)
        
    #     doc_chunk_sizes = []
    #     if use_hard_neg:
    #         counts = model_inputs[1]['tgt']['counts']
    #         assert len(counts)==qry_len, "Counts != qry_len"
            
    #         # cum = [0]
    #         # for c in counts:
    #         #     cum.append(cum[-1] + c)
                
    #         qry_chunks = self.split_inputs(model_inputs[0], self.chunk_sizes[0])

    #         tgt_chunks = []
    #         cum = 0
    #         j = 0
            
    #         for q_chunk in qry_chunks:
    #             chunk_len = len(q_chunk['qry']['text'])
    #             csum = sum(counts[j:j+chunk_len])
                
    #             slice_dict = {
    #                 k: (v[cum:cum+csum] if isinstance(v, list) else v)
    #                 for k, v in model_inputs[1]['tgt'].items()
    #             }
    #             doc_chunk_sizes.append(csum)
    #             slice_dict['counts'] = counts[j:j+chunk_len]
                
    #             tgt_chunks.append({'tgt': slice_dict})
    #             cum = cum + csum
    #             j = j + chunk_len
                

    #         model_inputs = [qry_chunks, tgt_chunks]
    #     else:
    #         model_inputs = [self.split_inputs(x, chunk_size) for x, chunk_size in zip(model_inputs, self.chunk_sizes)]
    ...
    #         if len(model_out)==3:
    #             batch_counts.extend(model_out[2])
                
    #         all_reps.append(model_reps)
    #         all_rnd_states.append(rnd_states)
        
    #     if batch_counts:
    #         loss_kwargs['batch_counts'] = batch_counts

    #     cache, loss = self.build_cache(*all_reps, **loss_kwargs)
        
    #     if use_hard_neg:
    #         _cache = [cache[0].split(self.chunk_sizes[0])]
    #         doc_chunked = []
    #         s = 0
    #         for sz in doc_chunk_sizes:
    #             doc_chunked.append(cache[1][s:s+sz])
    #             s = s + sz
    #         _cache.append(doc_chunked)
    #         cache = _cache
    #     else:
    #         cache = [c.split(chunk_size) for c, chunk_size in zip(cache, self.chunk_sizes)]
