...
from src.model_utils import LLAVA_NEXT, QWEN2_VL, PHI3V, GEMMA3, process_vlm_inputs_fns
...
def split_and_process_vlm_inputs(model_input: dict, chunk_size: int):
    ...
    for k in keys:
        ...
        elif isinstance(arg_val[k], str):
            chunked_tensor = arg_val[k]
        else:
...
def get_dense_rep(x):
    ...
    if x["qry_reps"] is None:
        if 'counts' in x:
            return x['tgt_reps'], x['counts']
        return x["tgt_reps"]
    else:
        if 'counts' in x:
            return x['qry_reps'], x['counts']
        return x["qry_reps"]
...
class TrainTextImageDataCollator:
    ...
    def __call__(self, examples):
        ...
        qry_inputs = self._get_batch_inputs(examples, "query_text", "query_image", self.data_args.Q_prompt)
        if 'neg_text' in examples[0]:
            cand_inputs = self._get_batch_candidates(examples, self.data_args.D_prompt)
        else:
            cand_inputs = self._get_batch_inputs(examples, "pos_text", "pos_image", self.data_args.D_prompt)
        
        return qry_inputs, cand_inputs #, neg_inputs

    def _get_batch_inputs(self, examples, text_keyname, image_keyname, system_prompt=None):
        texts, images = [], []
        ...
        if system_prompt is not None:
            inputs.update({'system': system_prompt})
            
        return inputs
    
    def _get_batch_candidates(self, examples, system_prompt=None):
        texts, images = [], [] # , counts, []
        for example in examples:
            # @ruimeng filter invalid data examples here will lead to fail to sync across devices (unequal batch size)
            # use dummy input for now
            cand_text = example['pos_text'] + example['neg_text']
            cand_image = example['pos_image'] + example['neg_image']
            cand_image = [ii if ii else None for ii in cand_image]
            texts.extend(cand_text)
            images.extend(cand_image)
            # counts.append(len(cand_text))
        
        inputs = {'text': texts, 'image': images}#, 'counts': counts}
...
@dataclass
class TrainTextNegativeCollator:
    data_args: DataArguments
    model_args: ModelArguments
    processor: ProcessorMixin
    ...
    def __call__(self, examples):
        ...
        :param examples: qry, qry_image, pos_text, pos_image
        """
        qry_inputs = self._get_batch_inputs(examples, "query_text", self.data_args.Q_prompt)
        cand_inputs = self._get_batch_candidates(examples, self.data_args.D_prompt)
        
        return qry_inputs, cand_inputs

    def _get_batch_inputs(self, examples, text_keyname, system_prompt=None):
        texts, images = [], []
        for example in examples:
            # @ruimeng filter invalid data examples here will lead to fail to sync across devices (unequal batch size)
            # use dummy input for now
    ...
    def _get_batch_candidates(self, examples, system_prompt=None):
        ...
        for example in examples:
            # @ruimeng filter invalid data examples here will lead to fail to sync across devices (unequal batch size)
            # use dummy input for now
            cand = example['pos_text'] + example['neg_text']
            texts.extend(cand)
            images.extend([None] * len(cand))
...
@dataclass
class TrainTextNegativeLateCollator:
    data_args: DataArguments
    model_args: ModelArguments
    processor: ProcessorMixin
    ...
    def __call__(self, examples):
        ...
        :param examples: qry, qry_image, pos_text, pos_image
        """
        qry_inputs = self._get_batch_inputs(examples, "query_text")
        qry_inputs = process_vlm_inputs_fns[self.model_args.model_backbone](qry_inputs,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len,
                                        system_prompt=self.data_args.Q_prompt,
                                        role='Q')
        
        cand_inputs, counts = self._get_batch_candidates(examples)
        cand_inputs = process_vlm_inputs_fns[self.model_args.model_backbone](cand_inputs,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len,
                                        system_prompt=self.data_args.D_prompt,
                                        role='D')
        cand_inputs.update({'counts': counts})
    ...
    def _get_batch_candidates(self, examples):
        texts, images, counts = [], [], []
        for example in examples:
            # @ruimeng filter invalid data examples here will lead to fail to sync across devices (unequal batch size)
            # use dummy input for now
...
class TrainJointDataCollator:
    ...
    def __call__(self, examples):
        ...
        :param examples: qry, qry_image, pos_text, pos_image
        """
        qry_inputs = self._get_batch_inputs(examples, "query_text", "query_image", contain_answer=True)
        qry_inputs = process_vlm_inputs_fns[self.model_args.model_backbone](qry_inputs,
                                        processor = self.processor,
                                        max_length = self.data_args.max_len)
    ...
    def _get_batch_inputs(self, examples, text_keyname, image_keyname, contain_answer=False):
        texts, images = [], []
        if contain_answer:
            answers = []
